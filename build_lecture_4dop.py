...
def draw_green_gap_pil():
    """«Зелёная щель»: зонная диаграмма InGaN + разрыв мощности 0,18 Вт vs 1000 Вт."""
    img = Image.new("RGB", (880, 560), "white")
    d = ImageDraw.Draw(img)
    ...
    _draw_text(
        d, (20, 500),
        "«Зелёная щель» (green gap): >30% индия → V-дефекты + QCSE → мощность ≤ 0,18 Вт. Нужен другой путь.",
        fill="#c62828", font=fs,
    )
    return img
...
SECTION_STAGE = {
    ...
    "4. ПРЯМАЯ ГЕНЕРАЦИЯ ЗЕЛЁНОГО СВЕТА: ПОЧЕМУ НЕ РАБОТАЕТ": (
        2.8,
        "Доска: формула энергии фотона E = hc/λ; схема зонной диаграммы InGaN; таблица «зелёная щель».",
    ),
...
SCRIPT = [
    ...
    ("2. ПЛАН ЛЕКЦИИ", "00:46 – 01:53 (стенка 67 сек = чтение 57 сек + доска/паузы 10 сек)", [
        ...
        "Третье. Прямая генерация зелёного света: почему полупроводниковые лазеры на нитриде галлия не могут дать нужную мощность. Это фундаментальное ограничение, называемое «зелёной щелью».",
    ...
    ("4. ПРЯМАЯ ГЕНЕРАЦИЯ ЗЕЛЁНОГО СВЕТА: ПОЧЕМУ НЕ РАБОТАЕТ", "06:41 – 12:08 (стенка 5,4 мин = чтение 2,6 мин + доска/паузы 2,8 мин)", [
        ...
        "Но здесь мы сталкиваемся с фундаментальным ограничением, которое в научной литературе называется «зелёной щелью» (green gap).",
    ...
    ("5. НЕЛИНЕЙНАЯ ОПТИКА: ГЕНЕРАЦИЯ ВТОРОЙ ГАРМОНИКИ", "12:08 – 19:50 (стенка 7,7 мин = чтение 4,0 мин + доска/паузы 3,7 мин)", [
        "Мы пойдём другим путём. Мы возьмём мощный и дешёвый инфракрасный лазер на 1070 нанометров и «превратим» его излучение в зелёное с помощью нелинейной оптики.",
        "Этот процесс называется генерацией второй гармоники (Second Harmonic Generation, SHG). Давайте разберём его математику.",
        "Почему в кристалле возникает нелинейность?",
        "В обычной среде — стекле, воздухе, воде — электроны колеблются симметрично относительно положения равновесия. Представьте шарик, лежащий на дне круглой чаши: если его толкнуть, он будет колебаться с той же частотой, что и толчок. Это линейный отклик.",
        "Но в нелинейном кристалле — например, в ниобате лития — ситуация иная. Потенциальная яма здесь — это график зависимости потенциальной энергии электрона от его смещения относительно ядра. В ниобате лития этот график асимметричен: с одной стороны яма круче, с другой — положе.",
        "Представьте шарик в чаше, у которой один край отвесный, а другой — пологий. Если толкнуть шарик, он будет колебаться несимметрично: смещение в одну сторону будет больше, чем в другую.",
        "Это приводит к тому, что отклик электрона на внешнее поле становится нелинейным — он содержит не только основную частоту, но и её гармоники.",
        "Математически это описывается степенным рядом: P = ε_{0}·(χ^{(1)}·E + χ^{(2)}·E^{2} + χ^{(3)}·E^{3} + …).",
        "Расшифровка каждого символа (та же таблица есть на слайде — с ней можно работать и без презентации): P — поляризация среды (Кл/м^{2}), дипольный момент единицы объёма; ε_{0} — электрическая постоянная, 8,85·10^{-12} Ф/м, где Ф (фарад) = Кл/В (кулон на вольт); E — напряжённость электрического поля волны (В/м); χ^{(1)} — линейная восприимчивость, безразмерная — отвечает за преломление; χ^{(2)} — квадратичная восприимчивость (м/В) — отвечает за удвоение частоты; χ^{(3)} — кубическая восприимчивость (м^{2}/В^{2}) — мала, ею пренебрегаем.",
        "Обратите внимание: индекс в скобках — это порядок нелинейности, а не степень числа.",
        "Почему мы пренебрегаем первым и третьим членами ряда? Первый член — линейный — описывает обычное преломление света. Он важен для понимания того, как волна распространяется в кристалле, но он не создаёт новых частот. Нас интересует именно рождение зелёного света, поэтому этот член мы не рассматриваем в рамках данной задачи.",
        "Третий член (χ^{(3)}) — кубическая нелинейность. Она отвечает за такие эффекты, как самофокусировка и четырёхволновое смешение. Однако для ниобата лития χ^{(3)} на несколько порядков меньше χ^{(2)}. При умеренных интенсивностях вкладом этого члена можно пренебречь. Он начинает играть роль только при экстремально высоких мощностях.",
        "Таким образом, второй член (χ^{(2)}·E^{2}) — это единственный член, который эффективно генерирует вторую гармонику в наших условиях. Именно его оставляем.",
        "Пусть на кристалл падает волна с частотой ω: E(t) = E_{0}·cos(ω·t). Здесь E_{0} — амплитуда поля; ω — циклическая частота (рад/с), связана с длиной волны формулой ω = 2πc/λ; t — время (с).",
        "Подставляем это поле во второй член разложения: P^{(2)}(t) ∝ χ^{(2)}·E_{0}^{2}·cos^{2}(ω·t). Знак «∝» читается «пропорционально». Он убирает постоянные множители, чтобы мы видели только форму колебания.",
        "Используем тригонометрическое тождество: cos^{2}(ω·t) = (1 + cos(2ω·t)) / 2.",
        "Физический вывод: первое слагаемое (1/2) — постоянное. Что оно означает физически? Это эффект оптического выпрямления (optical rectification). Под действием мощной световой волны в кристалле возникает постоянное электрическое поле — как если бы свет «выпрямился» и создал напряжение. Это поле не колеблется во времени, а значит, не излучает электромагнитные волны. Оно просто слегка смещает электроны в новое равновесное положение. Для нашей задачи генерации зелёного света это слагаемое бесполезно, и мы его игнорируем.",
        "Второе слагаемое cos(2ωt) означает, что среда колеблется с удвоенной частотой. Энергия фотона удваивается, а длина волны, согласно ω = 2πc/λ, уменьшается ровно в два раза: λ_{2ω} = 1070 нм / 2 = 535 нм.",
        "Теперь — ключевая проблема. Волна накачки (ω) и рождённая зелёная волна (2ω) движутся в кристалле с разными скоростями из-за дисперсии. Они рассинхронизируются, и зелёные волны, рождённые в разных точках кристалла, начинают гасить друг друга.",
        "Для решения этой проблемы мы используем периодически поляризованный ниобат лития (PPLN — Periodically Poled Lithium Niobate). Это кристалл, в котором через каждые Λ микрон направление кристаллической оси перевёрнуто на 180 градусов. Это меняет знак χ^{(2)} и компенсирует набег фазы.",
        "Условие синхронизма записывается так: Δk = k_{2ω} − 2k_{ω} = 2π/Λ. Где k — волновые числа; Λ (прописная греческая лямбда) — период решётки переполяризации (мкм). Для нашей задачи Λ ≈ 6,96 мкм.",
    ]),
    ("6. МОЁ ПРЕДЛОЖЕНИЕ: ДВУХПРОХОДНАЯ СХЕМА С ТЕРМОКОМПЕНСАЦИЕЙ", "19:50 – 25:42 (стенка 5,9 мин = чтение 2,8 мин + доска/паузы 3,1 мин)", [
    ...
    ("7. ИТОГОВЫЕ ТЕЗИСЫ", "25:42 – 27:28 (стенка 1,8 мин = чтение 1,8 мин)", [
        ...
        "Второе. Прямая генерация зелёного света на полупроводниках ограничена «зелёной щелью»: мощность не превышает 180 милливатт. Для промышленности нужны киловатты.",
    ...
    ("8. ЗАКЛЮЧЕНИЕ", "27:28 – 28:28 (стенка 1,0 мин = чтение 1,0 мин)", [
        "Итак, мы разобрали физику генерации зелёного излучения для аддитивных технологий. Ключевые моменты: медь требует зелёного света, прямая генерация ограничена «зелёной щелью», а нелинейная оптика позволяет обойти это ограничение.",
...
def build_pptx():
    ...
    bullets(s, [
        ...
        "3. Прямая генерация: «зелёная щель» InGaN",
    ...
    # ── Слайд 4: зелёная щель ──
    s = prs.slides.add_slide(prs.slide_layouts[6])
    hdr(s, "4. ПРЯМАЯ ГЕНЕРАЦИЯ: ПОЧЕМУ НЕ РАБОТАЕТ", ACCENT2, 24)
    bullets(s, [
        "InGaN-диоды существуют — но упираются в «зелёную щель» (green gap)",
    ...
    bullets(s, [
        ...
        "2. Прямая генерация: «зелёная щель», ≤ 0,18 Вт; нужны киловатты",
    ...
    bullets(s, [
        ...
        "Прямая генерация ограничена «зелёной щелью» InGaN",
...
def build_pdf():
    ...
    elements.append(Paragraph(
        ...
        "Строки «Доска / пауза:» — ремарки лектору, не читаются вслух. Целевой хронометраж лекции: 28,5 минуты.",
        hint,
    ))
